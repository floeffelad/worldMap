...
function setMarkers(map, locations) {
  ...
  for (var i = 0; i < locations.length; i++) {
    ...
    //google.maps.event.addListener(marker, "mouseover", function() {this.setIcon(imageOver);});
    //google.maps.event.addListener(marker, "mouseout", function() {this.setIcon(image);});

    //google.maps.event.addListener(marker, "mouseover", function(){ showCity(this, map)});

    //google.maps.event.addListener(marker, "mouseout", function (event) {infoboxCity.close(map, this);this.setZIndex(google.maps.Marker.MAX_ZINDEX - 1);});
...
function showCity(city, map) {
  ...
  var contentCity = [
    ['<div class="cityPolygon"><img src="fileadmin/worldMap/img/arrow.png"/></div>'],
...
function showGallery(city, map) {  
  ...
  //console.log(city);
  var content = [
          ['<div id="carousel-example-generic" class="carousel slide" data-ride="carousel"><div class="carousel-inner"></div><a class="left carousel-control" href="#carousel-example-generic" role="button" data-slide="prev"><button class="icon-arrow-left text-hide" style="display: inline-block; background-image: none; margin-top:185px;"><svg xmlns="http://www.w3.org/2000/svg" width="7.2" height="11.9" viewBox="-588 829.9 7.2 11.9"><path fill="#FFF" d="M-588 835.9c0-.2 0-.4.2-.5 0-.1.1-.1.1-.1l5.5-5.2c.3-.3.8-.3 1.1 0 .3.2.3.7 0 1l-5 4.7 5 4.7c.3.2.3.7 0 1-.3.3-.8.3-1.1 0l-5.6-5.2c-.2-.1-.3-.3-.2-.4z"></path></svg></button></span></a><a class="right carousel-control" href="#carousel-example-generic" role="button" data-slide="next"><button class="icon-arrow-right text-hide" style="display: inline-block; background-image: none; margin-top:185px;"><svg xmlns="http://www.w3.org/2000/svg" width="7.2" height="11.9" viewBox="-588 829.9 7.2 11.9"><path fill="#FFF" d="M-581.1 836.3l-5.6 5.2c-.3.3-.8.3-1.1 0-.3-.3-.3-.8 0-1l5-4.7-5-4.7c-.3-.3-.3-.8 0-1 .3-.3.8-.3 1.1 0l5.5 5.2s.1 0 .1.1c.2.1.2.3.2.5.1.1 0 .3-.2.4z"></path></svg></button></span></a></div>'],
  ...
  var infoboxOptionsGallery = {
   ...
   ,closeBoxURL: "fileadmin/worldMap/img/close.png"
   ,infoBoxClearance: new google.maps.Size(1, 1)
   ,isHidden: false
   ,pane: "floatPane"
  ...
  hr.onreadystatechange = function() {
    if(hr.readyState == 4 && hr.status == 200) {
      ...
      for(var o in d){
        if(d[o].src){
          $('.infoBox').find(".carousel-inner").append('<div class="item"><img src="'+d[o].src+'"><div class="carousel-caption"><h3>HERE COULD BE TEXT</h3></div></div>');
          $('.infoBox').find(".item:first").attr("class","item active");
        }
      console.log(content);
      }    
    }
  }
  hr.open("POST", "fileadmin/worldMap/json_gallery_data.php", true);
  ...
  console.log(infoboxGallery);
  infoboxGallery.addListener('domready', function() {
   // Set up automatic slide
